...
# import from nuclear-matter-convergence
import sys 
...
class SymmetryEnergyKernel(Kernel):
...
class ObservableContainer:
    ...
    def __init__(
            ...
            std, ls, ref, breakdown, err_y=0, derivs=(0, 1, 2), include_3bf=True, verbose=False, extend=False
    ):
        ...
        # offsetting data with non-constant mean function
#         def mean_n(x, c1, c2, c3, a, b, c):
#             # polytrope form, same for each order
#             self.c1 = c1
#             self.c2 = c2
#             self.c3 = c3
#             self.a = a
#             self.b = b
#             self.c = c
#             mean = c1*x**a + c2*x**b + c3*x**c
#             return mean
        
        def diff_mean_n(x, derivs=[0]):
            # calculate the derivatives needed (up to 2nd)
            diff_mean = np.zeros([len(x), len(derivs)])
            for i in range(len(x)):
                for n in range(len(derivs)):
                    if n == 0:
                        ...
                        #+ self.c2*self.b*(self.b-1) * x[i]**(self.b-2) + self.c3*self.c*(self.c-1) * x[i]**(self.c-2)
            return diff_mean
        
        def add_derivs(y_interp_vecs_n, diff_mean, derivs=[0]):
            # take the dict out of the dict and assign to arrays to manipulate
            y_plus_derivs = np.zeros([len(self.kf_interp), len(derivs)])
            for n in y_interp_vecs_n.keys():
                y_plus_derivs[:,n] = np.asarray(y_interp_vecs_n[n])
                for i in range(len(self.kf_interp)):
                    y_plus_derivs[i,n] += diff_mean[i,n]
                    
            # recast into a dict
            y_derivs_dict = {}
            for n in y_interp_vecs_n.keys():
                y_derivs_dict[n] = y_plus_derivs[:,n]
                            
            return y_derivs_dict

        # kernel initialization
        self.coeff_kernel = SquaredExponentialKernel(
        ...
        for i, n in enumerate(orders):
            ...
            self.kern_interp = kern_interp 
            self.kern_trunc = kern_trunc

            y_n = y[:, i]
            self._y_dict[n] = y_n
            
            # Interpolating processes
            #mu_n = ConstantMeanFunction(initial_params=[np.mean(y_n)])
            #mu_n = ConstantMeanFunction(initial_params=[np.max(y_n)+20])
            mu_n = ConstantMeanFunction(initial_params=[0])  # original one for mean 0 (keeping this)
            ...
            y_interp_all_derivs_n, cov_interp_all_derivs_n = predict_with_derivatives(
                gp=gp_interp, X=Kf_interp, n=derivs, return_cov=True   # here is the direct evaluation (would add back here?)
            )
            
            y_interp_vecs_n = get_means_map(y_interp_all_derivs_n, N_interp)

            # edit the above to add back the mean and any derivatives
            if extend is True:
                y_plus_derivs_n = add_derivs(y_interp_vecs_n, \
                                             diff_mean_n(self.kf_interp, derivs=[0,1,2]), [0,1,2])
            ...
            if extend is True:
                self._y_interp_vecs[n] = y_plus_derivs_n
            elif extend is False:
            ...
            gp_trunc = GaussianProcess(kern_trunc)
            self.gps_trunc[n] = gp_trunc

            cov_trunc_all_derivs_n = predict_with_derivatives(
                gp=gp_trunc, X=Kf_interp, n=derivs, only_cov=True    # size of this covariance matrix needed
            )
                        
            # add the covs here
            self.breakdown = breakdown
            hbarc = 197.327    # [Mev fm]
            kf_cutoff = self.breakdown/hbarc   # [fm^-1]
            
            # mask the trunc cov matrix for all negative values
            if extend is True:
                for i in range(len(cov_trunc_all_derivs_n)):
                    val = self.mask_array(cov_trunc_all_derivs_n[i,:], fill_value=4e14, fill_neg=True)
                    cov_trunc_all_derivs_n[i,:] = val  # could be val * i, in some way, to be kf dependent

            # add the cov matrices together linearly
            cov_total_all_derivs_n = cov_interp_all_derivs_n + cov_trunc_all_derivs_n
            
#             # print to a csv file for comparison
#             import csv

#             with open('output_old.csv', 'w') as f:
#                 writer = csv.writer(f)
#                 writer.writerows(cov_trunc_all_derivs_n)
                                        
            # covariance class variables
            self.cov_interp_all_derivs_n = cov_interp_all_derivs_n
            self.cov_trunc_all_derivs_n = cov_trunc_all_derivs_n
            self.cov_total_all_derivs_n = cov_total_all_derivs_n
            ...
            # make a class variable here for just truncation errors and pull the blocks per deriv
            cov_trunc_blocks_n = get_blocks_map(cov_trunc_all_derivs_n, (N_interp, N_interp))
            std_trunc_vecs_n = get_std_map(cov_trunc_blocks_n)
            self._std_trunc_vecs[n] = std_trunc_vecs_n
    ...
    def mask_array(self, array, cov=False, fill_value=None, fill_neg=False):
        ...
        # check for negative values and replace in mask
        if fill_neg is True:
    ...
    def get_trunc_std(self, order, deriv):
        std = self._std_trunc_vecs[order]
        return std[deriv]

    def draw_sample(self, order, num_samp=1, include_trunc=True):
        gp = GaussianProcess(k=self.gps_trunc[order].k)  # Kernel won't matter
...
class SymmetryEnergyContainer(ObservableContainer):
    ...
    def __init__(
        ...
        self.coeff_kernel_n = SquaredExponentialKernel(
        ...
        self.coeff_kernel_s = SquaredExponentialKernel(
        ...
        for i, n in enumerate(orders):
            ...
            mu_n = ConstantMeanFunction(initial_params=[0])
            gp_interp = GaussianProcess(kern_interp, mu=mu_n)
